...
//	}
	btn.innerHTML = '<a href="javascript:;" onclick="CloseLayer(); return false;" style="display:block;"><img id="pop-close-btn" src="/html/images/common/post_closed.png" alt="Close"></a>';
...
function ConfirmAction(obj) {
	if(confirm("Are you sure you want to " + obj.name + "?")) {
...
function iContent(url, tgtName, width, height) {
	...
	if(!editor) {
		alert("iContent Error :Please specify target.");
		return;
	}

	if(editor.cheditor.mode != "rich") { alert("Change to input mode."); return; }
	...
	} else { alert("File types that can not be inserted."); }
}
...
function ToggleLayer(objName, tarName, addX, addY) {
	...
	if(!obj) {
		alert(objName + ' layer does not exist.');
		return;
	}
...
function changeYear(element, d, num, limit) {
	...
	for (var i=year-num; i<=max; i++, j++) {
		element.options[j] = new Option(i + (add ? "year" : ""), i, false);
...
function checkDate(first, last) {
	if(first.value && last.value) {
		...
		if(edate < sdate) {
			var stitle = first.title || first.hname || " start date";
			var etitle = last.title || last.hname || " end date";
			alert(etitle + " must be equal to or later than " + stitle + ".");
			last.focus();
		} else {
			return true;
		}
...
function goCommonCart(ctype) {
	...
	if(!temp.join("|")) alert("No items selected.");
